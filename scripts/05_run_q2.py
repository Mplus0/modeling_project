...
from src.q2.optimizer import COST_LOCK_TOLERANCE, INITIAL_SOC, SOLVER_FEASTOL, VALIDATION_TOL, solve_actual, solve_plan, validate_schedule
...
def summarize(predictions, plan, actual, daily, windows):
    ...
    return {
        ...
        "maximum_cross_day_soc_violation": float(np.max(np.abs(daily.soc_start_kwh.to_numpy()[1:] - daily.soc_actual_end_kwh.to_numpy()[:-1]))),
        ...
        "solver_status_counts": {col: daily[col].value_counts().to_dict() for col in
                                 ["plan_primary_status", "plan_secondary_status", "actual_primary_status", "actual_secondary_status"]},
...
def run_q2(root=ROOT, skip_workbook=False):
    root = Path(root)
    ...
    frozen = sorted(set(list((root / "data/raw").rglob("*.xlsx")) + list((root / "data/processed").glob("*.csv"))
                        ...
                        + list((root / "outputs/figures/q1").glob("*")) + [root / "outputs/submissions/result1.xlsx"]))
    ...
    for day in sorted(all_days):
        ...
        if formal:
            ...
            # 日前两阶段优化完毕并冻结 G_plan 后，才读取当天实际负荷与光伏。
            observed = all_days[day]
            actual, am = solve_actual(observed["load"], observed["pv"], price, start_soc, commitment, day)
        ...
        if formal:
            ...
            row = {"date": day.isoformat(), "selected_load_window": selection["load_window_weeks"],
                   ...
                   "plan_secondary_throughput_kwh": pm["throughput"], "actual_secondary_throughput_kwh": am["throughput"],
            ...
            if len(daily) % 30 == 0 or day == FORMAL_DATES[-1]:
                print(f"Q2 已完成 {len(daily)}/334 天：{day}", flush=True)
        else:
    ...
    if len(actual) != 48096 or len(daily) != 334 or daily.soc_start_kwh.iloc[0] != 6000:
    ...
    for index, row in daily.iterrows():
        for mode, table in [("plan", plan), ("actual", actual)]:
            check = validate_schedule(table.iloc[index * 144:(index + 1) * 144], mode, row.soc_start_kwh,
...
if __name__ == "__main__":
    ...
    run_q2(skip_workbook=parser.parse_args().skip_workbook)
